...
class Test {
    ...
    fun wrapsCorrectly() {
        ...
        registerFactory(String::class) { a: BigInteger, b: BigDecimal -> "No param $a, $b" }
        assertEquals(with(scope("one")).provideSingletonFor<String>(BigInteger("2"), BigDecimal(2)), "No param 2, 2")
        registerFactory(String::class) { a: BigInteger, b: BigDecimal? -> "No param $a, $b" }
        val bigDecimal: BigDecimal? = null
        assertEquals(with(scope("one")).provideSingletonFor<String>(BigInteger("2"), bigDecimal), "No param 2, null")
    }
